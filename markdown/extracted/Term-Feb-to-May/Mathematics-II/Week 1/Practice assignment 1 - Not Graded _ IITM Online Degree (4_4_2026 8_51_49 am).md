...
If $A=\\begin{bmatrix} 2 & 2 \\\\ 0 & 2 \\end{bmatrix}$, then $A^{10} = 2^{10}\\begin{bmatrix} 1 & 10 \\\\ 0 & 1 \\end{bmatrix}$ (where $A^n$ is the matrix $A\\times A \\times \\ldots \\times A$, $n$\-times).
...
If the sum of the first and the third row vectors of a $3\\times 3$ matrix A is equal to the second row vector of $A$, then $det (A) = 0$.
...
Mahesh bought 2 kg potato and $c$ kg dal from a shop, and paid **₹** 200 to the shopkeeper. Gaurav bought 4 kg potato and 4 kg dal, and paid **₹** $d$ to the shopkeeper. If $x\_1$ represents the price of 1 kg potato and $x\_2$ represents the price of 1 kg dal, then choose the set of correct options.

The matrix representation to find $x\_1$ and $x\_2$ is $\\begin{bmatrix} 2 & 4 \\\\ 4 & c \\end{bmatrix} \\begin{bmatrix} x\_1 \\\\x\_2 \\end{bmatrix}=\\begin{bmatrix} 200 \\\\ d \\end{bmatrix}$

The matrix representation to find $x\_1$ and $x\_2$ is $\\begin{bmatrix} 2 & c \\\\ 4 & 4 \\end{bmatrix} \\begin{bmatrix} x\_1 \\\\ x\_2 \\end{bmatrix}=\\begin{bmatrix} 200 \\\\ d \\end{bmatrix}$
...
If bonus marks given to Safina, Ram and Pratiksha are represented by the following matrix $\\begin{bmatrix} 1.4 \\\\ 2.6 \\\\ 0 \\end{bmatrix}$ then the overall final grades (in 100) can be represented by the matrix:  
